...
struct BMPFileHeader {
  ...
  uint32_t offset_data; // Start position of pixel data (bytes from the
...
struct BMPInfoHeader {
  ...
  uint32_t compression; // 0 or 3 - uncompressed. THIS PROGRAM CONSIDERS ONLY
                        // UNCOMPRESSED BMP images
  uint32_t size_image;  // 0 for uncompressed images
  int32_t x_pixels_per_meter;
  int32_t y_pixels_per_meter;
  uint32_t colors_used; // No. color indexes in the color table. Use 0 for the
                        // max number of colors allowed by bit_count
  uint32_t colors_important; // No. of colors used for displaying the bitmap. If
                             // 0 all colors are required
};
...
struct BMP {
  ...
  void read(const char *fname) {
    ...
    if (inp) {
      ...
      // NOTE: 32bpp support is limited to BMPs that include V4/V5 masks
      // (tutorial-style). Some valid 32bpp BI_RGB BMPs (DIB size = 40) will be
      // rejected for now.
...
private:
  void read8bitColorPalette(std::ifstream &inp) {
    uint32_t palette_bytes =
        file_header.offset_data - info_header.size - sizeof(BMPFileHeader);
  ...
  void fileInfoTable() {
    std::cout << "bpp=" << info_header.bit_count << " dib=" << info_header.size
    ...
    printf("%c %c\n", file_header.file_type & 0xFF,
           (file_header.file_type >> 8) & 0xFF);
    printf("\n");
    printf("File size (HEX): ");
    printf("0x%08X\n",
           (file_header
                .file_size)); // We don't add the mask 0xFFFFFFFF here because
                              // the we are already expecting 32bits in %08X
  ...
  void check_color_header(BMPColorHeader &color_header) {
    ...
    if (expected_color_header.red_mask != color_header.red_mask ||
      ...
      throw std::runtime_error(
          "Unexpected color mask format! This program expects the pixel data "
          "to be in the BGRA format");
    }

    if (expected_color_header.color_space_type !=
        color_header.color_space_type) {
      throw std::runtime_error(
          "Unexpected color space type! The programa expects sRGB values");
    }
  }
